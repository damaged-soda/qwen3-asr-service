...
流式会话在等待起句时固定保留最近 300 毫秒音频；达到能量门限后，与当前帧一起立即送入模型，保护较轻的起音。缓冲按重采样后的 16kHz 样本计数，只回补尚未送入模型的音频；句开始时间包含回补范围。纯静音仍不触发识别，停止或重新配置会话会清空缓冲。此行为无需客户端配合，不增加等待计时，也不能恢复客户端未采集或未发送的声音。
