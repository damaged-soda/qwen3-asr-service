...
4. Server → per sentence `result-generated`: `payload.output.sentence` with `sentence_id`/`begin_time`/`end_time`(ms)/`text`/`sentence_end:true`/`words[]`. `sentence_id` starts at 0 per task and increments after each final; intermediate and final results for the same sentence share an ID. A new task on a reused connection resets the sequence to 0.
